...
class TestSecurityFixes:
    """Tests for security fixes identified in the audit."""
    
    def test_path_traversal_dotdot_blocked(self, temp_git_repo, git_state):
        """CRITICAL-SEC-002: Path traversal with .. should be blocked."""
        workspace, ralph_dir = temp_git_repo
        
        # Attempt path traversal with ..
        result = git_state._write_ref("../../../etc/passwd", "malicious")
        assert result is False, "Path traversal with .. should be blocked"
        
        # Verify file wasn't created outside .git
        assert not (workspace.parent / "etc" / "passwd").exists()
    
    def test_path_traversal_absolute_blocked(self, temp_git_repo, git_state):
        """CRITICAL-SEC-002: Absolute paths should be blocked."""
        workspace, ralph_dir = temp_git_repo
        
        # Attempt absolute path (blocked by _validate_ref_name)
        result = git_state._write_ref("/etc/passwd", "malicious")
        assert result is False, "Absolute paths should be blocked"
    
    def test_path_traversal_encoded_blocked(self, temp_git_repo, git_state):
        """CRITICAL-SEC-002: Encoded traversal attempts should be blocked."""
        workspace, ralph_dir = temp_git_repo
        
        # Attempt with consecutive slashes (blocked by validation)
        result = git_state._write_ref("ralph//iteration", "1")
        assert result is False, "Double slashes should be blocked"
    
    def test_symlink_in_path_blocked(self, temp_git_repo, git_state):
        """CRITICAL-SEC-002: Symlinks in ref path should be blocked."""
        workspace, ralph_dir = temp_git_repo
        
        # Create a symlink in .git directory
        git_dir = workspace / ".git"
        symlink_dir = git_dir / "ralph_link"
        ...
        try:
            ...
            result = git_state._write_ref("ralph_link/test", "value")
            assert result is False, "Writing through symlink should be blocked"
            
            # Verify file wasn't created through symlink
            assert not (target_dir / "test").exists()
        finally:
            if symlink_dir.exists():
                symlink_dir.unlink()
    
    def test_valid_ref_names_work(self, temp_git_repo, git_state):
        """Ensure valid ref names still work after security fixes."""
        workspace, ralph_dir = temp_git_repo
        
        # Valid ref names should work
        assert git_state._write_ref("ralph/iteration", "42")
        assert git_state._read_ref("ralph/iteration") == "42"
        
        assert git_state._write_ref("ralph/task", "TASK-001")
        assert git_state._read_ref("ralph/task") == "TASK-001"
    
    def test_increment_iteration_max_retries(self, temp_git_repo, git_state):
        """CRITICAL-REL-002: increment_iteration should not recurse infinitely."""
        workspace, ralph_dir = temp_git_repo
        
        # Test that normal operation still works
        n = git_state.increment_iteration()
        assert n >= 1, "increment_iteration should return valid number"
    
    def test_increment_iteration_normal_operation(self, temp_git_repo, git_state):
        """Ensure increment_iteration works normally after the fix."""
        workspace, ralph_dir = temp_git_repo
        
        # Multiple increments should work correctly
        n1 = git_state.increment_iteration()
        n2 = git_state.increment_iteration()
        n3 = git_state.increment_iteration()
